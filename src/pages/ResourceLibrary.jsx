...
export default function ResourceLibrary() {
	...
	const filterData = [
		...format.map((f) => ({ show: true, category: 'Media Type', title: f.title, id: f.id })),
		...stage.map((f) => ({ show: true, category: 'Project Stage', title: f.title, id: f.id })),
		...region.map((f) => ({ show: true, category: 'Region', title: f.title, id: f.id })),
		...industry.map((f) => ({ show: true, category: 'Industry', title: f.title, id: f.id }))
	];
	...
	useEffect(() => {
		...
		const mediaTypeFilters = selectedFilters.filter((f) => f.category === 'Media Type').map((f) => f.id);
		const projectStageFilters = selectedFilters.filter((f) => f.category === 'Project Stage').map((f) => f.id);
		const regionFilters = selectedFilters.filter((f) => f.category === 'Region').map((f) => f.id);
		const industryFilters = selectedFilters.filter((f) => f.category === 'Industry').map((f) => f.id);

		const filtered = allCourses.filter((course) => {
			const matchIndustry = industryFilters.length === 0 || industryFilters.includes(course.industry);
			const matchFormat = mediaTypeFilters.length === 0 || mediaTypeFilters.includes(course.format);
			const matchStage = projectStageFilters.length === 0 || projectStageFilters.includes(course.stage);
			const matchRegion = regionFilters.length === 0 || regionFilters.includes(course.region);
			const matchPrinciple = selectedPrinciple.length === 0 || selectedPrinciple.includes(course.principles);
			return matchFormat && matchStage && matchIndustry && matchRegion && matchPrinciple;
		});
		...
		<div className="container resource_page">
			...
			<div className="resource-carousel-container">
				...
				<div className="resource-carousel-wrapper">
					<div className="resource-carousel-track" style={{ transform: `translateX(-${index * 100}%)` }}>
						{slides.map((slide, i) => (
					...
					{slides.length > 0 && (
						<div className="resource-carousel-footer">
							<button
								className={`resource-carousel-btn icon-back ${index === 0 ? 'disabled' : ''}`}
							...
							<button
								className={`resource-carousel-btn icon-next ${(index === slides.length - 1 || slides.length == 0) ? 'disabled' : ''}`}
								onClick={() => goToSlide(index + 1)}
					...
					{slides.length === 0 && (
						<p className='no-resource'>Resources are not available for selected filter.</p>
					)}
				</div>
